...
abstract class PaginationPresenter implements PresenterInterface {
    ...
    public function getPageRange($start, $end)
    {
        ...
        for ($page = $start; $page <= $end; $page++)
        {
            ...
            {
                $pages[] = $this->getCurrentPageLink($page);
            }
            else
            {
    ...
    public function getPrevious($text = '&laquo;')
    {
        ...
        {
            return $this->getDisabledNavigationLink($text);
        }
        else
        {
            $url = $this->paginator->getUrl($this->currentPage - 1);

            return $this->getNavigationLink($url, $text);;
        }
    }
